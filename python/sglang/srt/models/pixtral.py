...
# from vllm.multimodal.inputs import torch.Tensor, PlaceholderRange
...
def _flatten_embeddings(embeddings: torch.Tensor) -> torch.Tensor:
    """
    Recursively flattens and concatenates torch.Tensor on all but the last
    dimension.
    """
...
class PixtralForConditionalGeneration(nn.Module):
    ...
    def get_multimodal_embeddings(self, forward_batch: ForwardBatch) -> torch.Tensor:
    ...
    def forward(
        ...
        if input_embeds is None:
            vision_embeddings = self.get_multimodal_embeddings(forward_batch)
    ...
    def _parse_and_validate_image_input(
        ...
        if (forward_batch.image_inputs is not None):
            images = forward_batch.image_inputs
...
EntryClass = PixtralForConditionalGeneration